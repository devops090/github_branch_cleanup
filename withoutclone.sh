...
while read -r line; do
    if [ line=$(awk -F ',' '{print $1}' repo_branch_list.csv) ]; then
         name_repo="$line"
         echo "Name read from repo - $name_repo"
    fi
    if [ line=$(awk -F ',' '{print $2}' repo_branch_list.csv) ]; then
         name_branch="$line"
         echo "Name read from branch - $name_branch"
    fi
